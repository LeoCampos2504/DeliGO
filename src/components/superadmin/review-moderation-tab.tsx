...
export function ReviewModerationTab() {
  ...
  return <div className="space-y-4">
    ...
    {list.isLoading ? <p className="text-sm text-muted-foreground">Cargando solicitudes…</p> : <div className="grid gap-3">
      {items.map((item) => <button type="button" key={item.id} onClick={() => setSelected(item.id)} className="rounded-xl border p-4 text-left transition hover:bg-muted/40"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-semibold">{item.negocio.nombre}</p><p className="text-xs text-muted-foreground">{item.motivo} · {new Date(item.createdAt).toLocaleDateString("es-AR")}</p></div><Badge variant="secondary">{labels[item.estado]}</Badge></div><p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{item.resena.comentario}</p><p className="mt-2 text-xs text-muted-foreground">Puntuación: {item.resena.puntuacion}/5 · Vence: {new Date(item.venceEn).toLocaleDateString("es-AR")}</p></button>)}
    ...
    <Dialog open={!!selected} onOpenChange={(isOpen) => { if (!isOpen) { setSelected(null); setAction(null); setText("") } }}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl">
        ...
        {detail.isLoading ? <p>Cargando expediente…</p> : detail.data && <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-2"><Badge>{labels[detail.data.estado]}</Badge><span className="text-xs text-muted-foreground">{detail.data.negocio.nombre}</span></div>
          <section className="rounded-lg border p-3 text-sm"><p className="font-medium">Motivo: {detail.data.motivo}</p><p className="mt-1 whitespace-pre-wrap text-muted-foreground">{detail.data.explicacionOriginal}</p></section>
          <section className="rounded-lg border p-3 text-sm"><p className="font-medium">Reseña · {detail.data.resena.puntuacion}/5</p><p className="mt-1 whitespace-pre-wrap text-muted-foreground">{detail.data.resena.comentario}</p>{detail.data.resena.respuestaNegocio && <p className="mt-2 text-muted-foreground">Respuesta del negocio: {detail.data.resena.respuestaNegocio}</p>}</section>
          {detail.data.motivoDecision && <section className="rounded-lg border p-3 text-sm"><p className="font-medium">Motivo de la decisión</p><p className="mt-1 whitespace-pre-wrap text-muted-foreground">{detail.data.motivoDecision}</p></section>}
          <section className="space-y-3 border-l pl-3">{detail.data.eventos.map((event) => <div key={event.id} className="space-y-1 text-sm"><p className="font-medium">{eventLabels[event.tipo] ?? "Actualización"}</p>{event.mensaje && <p className="whitespace-pre-wrap text-muted-foreground">{event.mensaje}</p>}<p className="text-xs text-muted-foreground">{new Date(event.createdAt).toLocaleString("es-AR")}</p><EvidenceList solicitudId={detail.data.id} eventId={event.id} evidencias={event.evidencias} /></div>)}</section>
