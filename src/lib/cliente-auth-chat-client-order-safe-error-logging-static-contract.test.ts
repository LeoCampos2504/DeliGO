...
// ============================================
// GLOBAL-LOGS-PII-2B — contrato estático: Cliente/Auth/Chat/pedidos-checkout
// del cliente nunca vuelven a loguear un error atrapado sin pasar por
// `safeErrorForLog`
// ============================================
// Mismo criterio que negocio-api-safe-error-logging-static-contract.test.ts
// (GLOBAL-LOGS-PII-2A): lectura real de código fuente + aserciones
// puntuales — nunca un parser completo de TypeScript. El heurístico (¿el
// argumento de un console.* es el parámetro de la `catch` más cercana, y no
// está envuelto en `safeErrorForLog(...)`?) fue validado manualmente contra
// los 49 callers reales migrados en GLOBAL-LOGS-PII-2B antes de convertirlo
// en test permanente — cero falsos positivos en esa corrida.
//
// Acotado a propósito a las 4 rutas de esta tarea:
// - src/app/api/cliente/**
// - src/app/api/auth/**
// - src/app/api/chat/**
// - src/app/api/pedidos/** + src/app/api/public/mesa-geofence,
//   src/app/api/public/mesa-cuenta (pedidos/checkout del cliente fuera de
//   negocio/**, descubiertos — no asumidos — en GLOBAL-LOGS-PII-2B)
//
// Fuera de scope a propósito: negocio/**, operativo/**, operaciones/**,
// superadmin/**, repartidor/**, y el resto de src/app/api/public/** (no
// forma parte de los 4 dominios nombrados en esta tarea).
...
const SCOPE_DIRS = [
  join(process.cwd(), "src", "app", "api", "cliente"),
  join(process.cwd(), "src", "app", "api", "auth"),
  join(process.cwd(), "src", "app", "api", "chat"),
  join(process.cwd(), "src", "app", "api", "pedidos"),
  join(process.cwd(), "src", "app", "api", "public", "mesa-geofence"),
  join(process.cwd(), "src", "app", "api", "public", "mesa-cuenta"),
]
...
describe("GLOBAL-LOGS-PII-2B — contrato estático: Cliente/Auth/Chat/pedidos-checkout nunca loguean errores crudos", () => {
  test("ningún console.* en el scope de 2B pasa un error atrapado sin safeErrorForLog(...)", () => {
    const violations = findUnsafeErrorLogCalls()
    if (violations.length > 0) {
      const details = violations.map((v) => `  ${v.file}:${v.line} -> ${v.snippet}`).join("\n")
      throw new Error(`Se encontraron ${violations.length} caller(s) de error crudo en el scope de GLOBAL-LOGS-PII-2B:\n${details}`)
    }
