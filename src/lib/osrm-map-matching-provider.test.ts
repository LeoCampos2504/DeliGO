...
describe("P2-T24 OSRM adapter", () => {
  ...
  test("emits bounded Testing timing stages and safe request characteristics on success", async () => {
    ...
    const provider = createOsrmMapMatchingProvider({
      ...
      enabled: true,
      env: { DELIGO_ENVIRONMENT: "TESTING", NODE_ENV: "test" },
      timingLogger: (event) => events.push(event),
    ...
    expect(end.fetchHeadersMs).toBeDefined()
    expect(end.jsonBodyParseMs).toBeDefined()
    expect(end.totalProviderMs).toBeDefined()
    expect(end.abortElapsedMs).toBeUndefined()
    ...
    expect(end.requestPathKind).toBe("match")
    expect(end.geometriesMode).toBe("geojson")
    expect(end.overviewMode).toBe("full")
    expect(end.gapsMode).toBe("split")
    expect(end.tidyMode).toBe("false")
    expect(end.serializedRequestLengthBytes).toBeGreaterThan(0)
  })
  ...
  test("emits abort elapsed timing on timeout without coordinates, URL, or PII", async () => {
    ...
    expect(end.jsonBodyParseMs).toBeUndefined()
    expect(end.totalProviderMs).toBeDefined()
    ...
    expect(serializedEvents).not.toContain("example.com")
    expect(serializedEvents).not.toContain("pedidoId")
    expect(serializedEvents).not.toContain("clienteId")
    expect(serializedEvents).not.toContain("repartidorId")
    expect(serializedEvents).not.toContain("negocioId")
    expect(serializedEvents).not.toContain("trajectory")
  })
  ...
  test("allows the optimized standalone Testing runtime despite NODE_ENV=production", async () => {
    ...
    const provider = createOsrmMapMatchingProvider({
      baseUrl: "https://example.com",
      enabled: true,
      env: { DELIGO_ENVIRONMENT: "TESTING", NODE_ENV: "production" },
      timingLogger: (event) => events.push(event),
