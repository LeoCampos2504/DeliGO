...
// PUT - Toggle or set promocionado on a negocio
export async function PUT(
  ...
  try {
    ...
    const negocio = await db.negocio.findUnique({
      where: { id },
      select: { id: true, promocionado: true },
    })
    ...
    const { promocionado, ordenPromocion } = body
    ...
    const updateData: { promocionado: boolean; ordenPromocion?: number } = {
      promocionado,
    }
