...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return response({ ok: true, ...(await mutateReviewModerationRequest({ solicitudId: id, superadminId: auth.admin.id, action: "APROBAR", text: motivoDecision })) })
  } catch (error) {
    ...
    console.error("Error approving review moderation request:", safeErrorForLog(error))
