...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const mensaje = parseReviewModerationDecisionBody(raw, "mensaje")
    if (!mensaje) return response({ error: "Body inválido" }, { status: 400 })
    ...
    return response({ ok: true, ...(await mutateReviewModerationRequest({ solicitudId: id, superadminId: auth.admin.id, action: "PEDIR_INFORMACION", text: mensaje })) })
  } catch (error) {
    ...
    console.error("Error requesting review moderation information:", safeErrorForLog(error))
