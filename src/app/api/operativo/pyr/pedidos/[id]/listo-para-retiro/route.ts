...
//   preparando → listo_para_retirar  para un pedido de RETIRO del negocio autorizado.
...
// TRANSICIONES confirma preparando → listo_para_retirar EXCLUSIVAMENTE para "retiro" (no
// existe para "domicilio"). Esta ruta personal reutiliza EXACTAMENTE, para el caso fijo
// preparando→listo_para_retirar de retiro: el mismo CAS (id + negocioId +
// metodoEntrega:"retiro" + estado="preparando"), la misma auditoría
...
const CONFLICT_MESSAGE = "El pedido ya no está disponible para marcarlo listo para retirar."
...
export async function POST(
  ...
  try {
    ...
    // 4) CAS atómico: solo si sigue en "preparando" y es de retiro. La decisión final es
    ...
    try {
      ...
      console.error("[OperativoPyR] Falló la auditoría de marcado como listo para retirar")
    }
    ...
    if (pedido.clienteId) {
      try {
        ...
        const payload = orderUpdateNotification(id, pedido.negocioNombre, "listo_para_retirar")
        ...
        console.error("[OperativoPyR] Falló la notificación de listo para retirar")
      }
    }
