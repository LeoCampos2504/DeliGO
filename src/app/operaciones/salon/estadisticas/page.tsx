...
// ============================================
// Tipos (espejo de las estadísticas seguras de Salón)
// ============================================
...
function toResumen(raw: unknown): Resumen {
  ...
  return {
    ...
    pedidosEntregados: num(r.pedidosEntregados),
    pedidosCancelados: num(r.pedidosCancelados),
...
// Página
// ============================================
export default function OperacionesSalonEstadisticasPage() {
  ...
  const refresh = useCallback(
    async (p: Periodo) => {
      ...
      try {
        const res = await fetch(`/api/operaciones/salon/estadisticas?periodo=${p}`, {
          cache: "no-store",
          signal: ac.signal,
        })
  ...
  if (phase.kind === "no-permission") {
    return (
      <CenteredShell>
        ...
        <p className="text-sm text-muted-foreground max-w-xs">
          Esta terminal no tiene permiso para acceder a las estadísticas de Salón.
        </p>
        <Button asChild className="rounded-xl gap-2 font-semibold">
      ...
      <div className="max-w-5xl mx-auto px-4 py-4 space-y-4 pb-24">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <MetricCard
            ...
            hint="Pedidos de mesa entregados o cancelados en el período."
          />
          <MetricCard
            icon={CheckCircle2}
            tone="emerald"
            label="Entregados"
            value={r.pedidosEntregados}
            hint="Pedidos de mesa que se completaron y entregaron."
          />
          <MetricCard
            icon={XCircle}
            tone="red"
            label="Cancelados"
            value={r.pedidosCancelados}
            hint="Pedidos de mesa que fueron cancelados."
          />
          <MetricCard
            icon={Percent}
            tone="primary"
