...
P2-T50 — Salon Statistics Custom Date Filtering — P2 — AUDITED_DESIGNED_READY_FOR_IMPLEMENTATION
...
14. P2-T50 — SALON STATISTICS CUSTOM DATE FILTERING — AUDITED_DESIGNED_READY_FOR_IMPLEMENTATION (P2 — A0 2026-09-20 confirmó que la pantalla objetivo es el dashboard de Negocio (`/negocio` → `SalonTab` → `EstadisticasSubTab`, backend `GET /api/negocio/salon/stats`), NO la pantalla homónima pero distinta de Terminal (`/operaciones/salon/estadisticas`, hoy/7d/30d, sólo conteos) ni el panel personal (que no tiene estadísticas en absoluto); `T50_API_DECISION=B` — endpoint reutilizable con parámetros aditivos (`fecha`/`mes`/`desde`+`hasta` junto al `periodo` existente), sin endpoint nuevo; halló una inconsistencia preexistente no reportada antes: el badge `mesasAsignadas` por mozo es un snapshot en vivo sin filtro de fecha, a diferencia de las demás 4 métricas que sí comparten el mismo `dateFilter` — documentada, no corregida (fuera de alcance); modelo de rango recomendado FROM inclusivo/TO exclusivo, sin nueva librería de fechas (usar aritmética nativa `Date` como ya hace el propio repo), UI reutilizando `Calendar`/`Popover` ya instalados y sin uso previo; cero código de producto, cero DB, cero schema; ver P2_T50_A0_SALON_STATISTICS_DATE_FILTER_AUDIT_DESIGN.md; punto 9)
