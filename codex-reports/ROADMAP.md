...
# CURRENT AUTHORITATIVE BACKLOG — P2-T23-R3A (2026-09-13)

Esta cabecera supersede el snapshot R2A para el estado actual. R3A implementa
el productor bounded y el contrato HTTP/realtime en Testing; R3B conserva el
playback confirmado y el harness para la siguiente tarea.
...
Reporte vigente: `codex-reports/P2_T23_R3A_BOUNDED_TRAJECTORY_TRANSPORT_IMPLEMENTATION.md`.

# CURRENT AUTHORITATIVE BACKLOG — P2-T23-R2A (2026-09-13)

Esta cabecera supersede el snapshot anterior de T23. La aclaración de producto
requiere conservar puntos GPS intermedios reales; A0 queda superseded para la
implementación futura, y el R2 de interpolación no committeado fue descartado.
...
P2_T54_STATUS=FUTURE_AFTER_T02_T23_T24
NEXT_TASK_CHAIN_START=P2_T23_R3A
PUBLIC_RELEASE_AUTHORIZED=NO
```

Reporte vigente: `codex-reports/P2_T23_R2A_TRAJECTORY_BATCHING_ARCHITECTURE_PIVOT.md`.
...
P2_T02_B4_ANDROID_STATUS=PASS
P2_T02_B4_STATUS=CROSS_PLATFORM_CERTIFIED
P2_T02_B3_STATUS=CLOSED_TESTING_CERTIFIED
P2_T02_RELEASE_ELIGIBLE=SI
...
P2_T23_STATUS=FUTURE_DEPENDS_ON_P2_T02_NAV_SCOPE_PARTIAL
P2_T24_STATUS=FUTURE_DEPENDS_ON_P2_T23_NOT_STARTED
P2_T54_STATUS=FUTURE_AFTER_T02_T23_T24
P2_T54_TITLE=Driver Turn-by-Turn Follow Camera UX
...
## P2-T02-R6 — final physical evidence reconciliation and closeout (2026-09-13)

La evidencia física reciente cubre los gates de movimiento bajo y sobre umbral,
recorrido exterior real de kilómetros, stationary control, background
best-effort con stale UI honesta y detención posterior a Entregado. El marcador
salta entre puntos sin interpolación: queda confirmado para T23 y relacionado
parcialmente con T24, pero no bloquea T02 porque no es un requisito de cierre
del tracking actual.
...
P2_T02_RELEASE_ELIGIBLE=SI
TEST_T02_ORDER_LEFT_ACTIVE=NO
NEXT_ACTION=P2_T23_AUDIT_AND_DESIGN
...
el runtime exacto desplegado en Testing. Esto no cierra T02 global: Android,
movimiento exterior, finalización y tracking posterior a la entrega siguen
...
P2-T54 queda formalizada como tarea futura y bloqueada por la secuencia
...
## P2-T02-R5B — Client tracking safe-area operator certification closeout (2026-09-12)

El operador certificó físicamente R5A en iPhone/PWA y realizó el smoke visual
en Android. Se cierra únicamente R5A en Testing; T02 global conserva sus gates
de movimiento exterior, Android GPS y finalización.
...
R5A queda cerrada y no se reabre. No se inician T23, T24 ni T54.
...
P2_T50_STATUS=CLOSED_TESTING_CERTIFIED / RELEASE_ELIGIBLE=YES /
  PRODUCTION_PROMOTED=NO (filtro de fecha personalizado día/mes/rango en
  estadísticas de Salón del dashboard de Negocio, extensión aditiva de
...
```text
P2-T47 — Operations Product Personalization UX — P2_UX — AUDITED_DESIGNED_READY_FOR_IMPLEMENTATION
...
NEXT_RECOMMENDED_SOFTWARE_TASK_REASON=P2-T49 y P2-T50 cerraron CLOSED_TESTING_CERTIFIED (2026-09-20) — no queda ningún P1 accionable en el backlog activo. T47 completó su auditoría/diseño A0 (2026-09-20, ver P2_T47_A0_PRODUCT_PERSONALIZATION_UX_AUDIT_DESIGN.md) y queda `AUDITED_DESIGNED_READY_FOR_IMPLEMENTATION` — es la siguiente tarea activa sin dependencia externa ni hardware, a diferencia de T23/T24/T34 (bloqueadas por decisión/hardware).
...
13. P2-T47 — OPERATIONS PRODUCT PERSONALIZATION UX — AUDITED_DESIGNED_READY_FOR_IMPLEMENTATION (P2_UX — A0 2026-09-20 identificó el target real: `ProductConfigurator`, inline en `src/app/mozo/panel/[slug]/pedido/[mesaId]/page.tsx`, compartido vía re-export literal con `/operaciones/mi-panel/[slug]/pedido/[mesaId]` (Mozo y Operaciones personal son literalmente el mismo archivo); Terminal Salón kiosco NO tiene ningún flujo de personalización de producto (`OPERACIONES_AREAS=["salon","pyr"]`, sin scope de pedidos); root cause de "Aderezos/aderezos" confirmado con evidencia exacta de código: `Agregado.categoria`/`Ingrediente.categoria` son texto libre sin constraint en el schema, la UI de catálogo del Negocio (`agregados-section.tsx`/`ingredientes-section.tsx`) hace un dedup case-sensitive (`Array.includes`) al crear categorías nuevas, y el agrupamiento por categoría en `ProductDetailSheet` (Cliente storefront `/n/[slug]`, Cliente Mesa `/n/[slug]?mesa=X`, y la Vista previa del Negocio) usa un `Map` sin normalizar — dos categorías que difieren sólo en mayúsculas se renderizan como dos encabezados separados; `ProductConfigurator` (el personalizador real de Operaciones) NUNCA agrupa por categoría, así que estructuralmente no puede reproducir ese síntoma específico — el finding original mezcló la inconsistencia visual general (target: `ProductConfigurator`) con la duplicación Aderezos/aderezos (target real: `ProductDetailSheet`, alcanzado como Cliente Mesa durante la misma revisión física, no como "Operaciones" en sentido estricto); `ADEREZOS_DUPLICATION_CLASSIFICATION=DATA_INCONSISTENCY`; R1 diseñado como 100% frontend (cero API/schema/migración): rediseño visual de `ProductConfigurator` (Badge "Obligatorio", contador N/max, variante roja/tachada para quitar ingredientes, `aria-pressed`, safe-area en el bottom bar) + normalización case-insensitive del agrupamiento en `ProductDetailSheet` (conservando el primer texto visto como label); el fix del dedup en el catálogo del Negocio queda documentado como hallazgo separado, fuera de este backlog item; ver P2_T47_A0_PRODUCT_PERSONALIZATION_UX_AUDIT_DESIGN.md; punto 5)
