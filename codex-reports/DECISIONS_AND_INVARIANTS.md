...
## P2-T23-R3A — contrato implementado en Testing (2026-09-13)
...
La revisión del cliente visible se mantiene legacy hasta R3B; el transporte no
se presenta como certificación de smooth playback.

## P2-T23-R2A — producto y arquitectura vigentes
...
La arquitectura anterior de interpolación-only queda preservada como historial,
no como decisión de implementación vigente.
...
## P2-T02-R6 — final physical certification invariants (2026-09-13)
...
El contrato físico de B3 se satisface como PWA best-effort: no se exige
continuidad garantizada en background, pero sí que el Cliente deje de afirmar
“En vivo” cuando la última coordenada envejece. El comportamiento stepwise del
marker no se convierte en una promesa de interpolación; se preserva como
alcance de T23/T24. El seguimiento se corta de forma terminal al completar la
entrega.

Reporte: `codex-reports/P2_T02_R6_FINAL_PHYSICAL_EVIDENCE_RECONCILIATION.md`.

## P2-T23-A0 — smooth trajectory playback design invariants (2026-09-13)
...
La reproducción propuesta retargetea desde la posición renderizada al punto
confirmado más nuevo, deduplica la misma coordenada y revisión, hace snap tras
stale/reconnect/gap grande y cancela rAF/timers al completion. No se crea un
segundo watcher ni se absorben T24 o T54.

Reporte: `codex-reports/P2_T23_A0_TRAJECTORY_BATCHING_SMOOTH_PLAYBACK_AUDIT_DESIGN.md`.
...
## P2-T40-R1 — Push session reconciliation invariants — 2026-09-20

```text
MODEL_C1_MULTI_BIND=CONFIRMADO_CORRECTO_NO_TOCADO
PHYSICAL_UNSUBSCRIBE_POLICY=SERVER_DETACH_ONLY_NO_TOCADO
STALE_PREVIOUS_OWNER_CLEANUP_TRIGGER=NEXT_LOGIN_SAME_SLOT_NUNCA_401_HANDLER
STALE_OWNER_IDENTITY_SOURCE=SERVER_SIDE_HANDOFF_COOKIE_JWT_NUNCA_INPUT_DE_CLIENTE
CROSS_FAMILY_MULTI_BIND=PRESERVADO_NUNCA_LIMPIADO
MANUAL_OFF_POLICY=M2_DEVICE_SCOPED_WITH_NEXT_LOGIN_REENABLE_OFFER
MANUAL_OFF_STORAGE=localStorage_owner_scoped_sin_PII_sin_schema_change
USE_AUTH_401_HANDLER_TOUCHED=NO (A1 lo prohibió explícitamente)
GOOGLE_OAUTH_HANDOFF_MECHANISM=MISMO_QUE_PASSWORD_LOGIN_SIN_CANAL_ADICIONAL
PUSH_OWNER_HANDOFF_SECRET=NUEVO_SECRETO_DEDICADO_TTL_120S_FAIL_OPEN_SIN_EL
```

Decisión de producto autorizada por el operador en el propio prompt de R1
(reemplaza `PRODUCT_DECISION_REQUIRED=SI` de A1 por
`PRODUCT_DECISION_RESOLVED=SI`): un apagado manual de Push permanece
desactivado en el dispositivo hasta reactivación explícita, incluso entre
sesiones (`MANUAL_OFF_AUTO_REBIND_ON_NEXT_LOGIN=NO`), pero DeliGO ofrece
